...
/**********************************************************************************************
  ...
  YOU FURTHER ACKNOWLEDGE AND AGREE THAT THE SOFTWARE AND DOCUMENTATION ARE PROVIDED УAS ISФ
...
void main (void)
{
...
#ifdef I_WANT_TO_CHANGE_DEFAULT_ROM_DEVICE_ADDRESS_PSEUDO_CODE
  {
	...
	const addr_t lAddr = {0x00, 0x00, 0x01, 0x00};
	// set device address
	...
	// ѕсевдопрерывание от радиопередачи по аналогу с UART IRQ
	radio_processing();
  ...
  while (1)
  {
    ...
    {
    	uint8_t     len;
    	unsigned char i,msg[MAX_APP_PAYLOAD];
      ...
      for (i=0; i<sNumCurrentPeers; ++i)
      {
        if (SMPL_SUCCESS == SMPL_Receive(sLID[i], msg, &len))
        {
   		 ...
   		 {
   			 rf_rx_bufer[0] = msg[0];
   			 rf_rx_bufer[1] = msg[1]+0x30;
   		 }
			tx_send_wait("\r\nED",sizeof("\r\nED"));
			tx_send_wait(&(rf_rx_bufer[1]),1);	// ѕередаем по UART прин€тое по радиоканалу сообщение
			tx_send_wait(" message: ",sizeof(" message: "));
			tx_send_wait(&(rf_rx_bufer[0]),1);
